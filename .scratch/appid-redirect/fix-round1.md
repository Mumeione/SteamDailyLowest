...
> 来源：`debug_2026_09_25.md`（2026-09-25 检查报告）
...
- **把 `debug_2026_09_25.md` 移进 `.scratch/<slug>/`**（Standards 轴 (a)）：该报告同时覆盖
  P0 / 两个 P1 / P2，不是单一 feature 的 spec 或 issue，AGENTS.md 的目录约定不覆盖它。
  留在仓库根，与 README / AGENTS 同级。
...
- P1 `report.py::build_groups` 组内 `start_text` 抖动导致详情区多一行（121 张卡）
- P1 `report.py::format_amount` 砍尾零（`¥12.7` 应为 `¥12.70`）
- P2 `.github/workflows/daily.yml` 补跑 guard 窗口与间隔矛盾（决策项）